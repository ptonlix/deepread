...
2. **管道集成**  
   ...
   - 支持两种模式：  
     - *本地模式*: 使用vLLM Python API（`LLMEngine` / `AsyncLLMEngine`）加载DeepSeek-OCR模型，直接在进程内推理。  
     - *远程模式*: 调用基于vLLM Inference部署的官方server（FastAPI+Uvicorn），参数包括服务URL、超时、重试次数。  
...
- 推理日志与监控的集成方式（例如Prometheus指标、请求追踪）。*** End Patch***
